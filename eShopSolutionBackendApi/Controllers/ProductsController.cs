...
{   //api/products
    ...
    public class ProductsController : ControllerBase 
    {
        ...
        //http://localhost/product?pageIndex=1&pageSize=10&CategoryId=
        [HttpGet("{languageId}")]
        ...
        public async Task<IActionResult> GetById(int iproductId, string languageId)
        {
            var product = await _manageProductService.GetById(iproductId, languageId);
            if (product == null)
        ...
        public async Task<IActionResult> GetImageById(int iproductId, int imageId)
        {
            var image = await _manageProductService.GetImageById(imageId);
            if (image == null)
                return BadRequest("Cannot find product");
